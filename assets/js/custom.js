!(function ($) {
  ...
  Sofash.prototype.initScrollspy = function () {
    // Vérifie si le sélecteur est valide
    var targetSelector = "#navbar";
    var $target = $(targetSelector);

    if ($target.length > 0) {
      // Initialisation de scrollspy
      $("#navbarCollapse").scrollspy({
        target: targetSelector,
        offset: 70,
      });

      // Gestion des erreurs pour éviter le problème de syntaxe
      $("body").on("activate.bs.scrollspy", function () {
        $("[data-ignore-scrollspy]").each(function () {
          $(this).removeClass("active");
        });
      });
    } else {
      console.error(
        "Le sélecteur de cible pour scrollspy n'est pas valide : " +
          targetSelector
      );
    }
  };
